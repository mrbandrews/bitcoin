...
def test_unconfirmed_not_spendable(rbf_node, rbf_node_address):
    ...
    assert_equal([t for t in rbf_node.listunspent(minconf=0, include_untrusted=False) if t["txid"] == bumpid], [])
    ...
    assert_equal([t for t in rbf_node.listunspent(minconf=0, include_untrusted=False) if t["txid"] == rbfid], [])

    # check that the main output from the rbf tx is spendable after confirmed
    rbf_node.generate(1)
    assert_equal(
        sum(1 for t in rbf_node.listunspent(minconf=0, include_untrusted=False)
