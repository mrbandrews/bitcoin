...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw runtime_error(
            "listunspent ( minconf maxconf  [\"addresses\",...] [include_unspendable] )\n"
            ...
            "      ,...\n"
            "    ]\n"
            "4. include_unspendable (bool, optional, default=true) Include untrusted transactions.\n"
            "\nResult\n"
    ...
    bool include_unspendable = true;
    if (request.params.size() > 3 && !request.params[3].isNull()) {
        RPCTypeCheckArgument(request.params[3], UniValue::VBOOL);
        include_unspendable = request.params[3].get_bool();
    }
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    pwalletMain->AvailableCoins(vecOutputs, !include_unspendable, NULL, true);
...
{ //  category              name                        actor (function)           okSafeMode
    ...
    { "wallet",             "listunspent",              &listunspent,              false,  {"minconf","maxconf","addresses","include_unspendable"} },
